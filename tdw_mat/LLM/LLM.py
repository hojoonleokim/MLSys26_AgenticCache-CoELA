...
class LLM:
    def __init__(self,
        ...
        self.max_frames = getattr(sampling_parameters, 'max_frames', 3000)
                ...
                try:
                    if 'gpt-5' in self.lm_id or 'gpt-4.1' in self.lm_id:
                        ...
                        if self.debug:
                            ...
                            # Additional detailed logging when debug is enabled (matches temp/LLM.py style)
                            with open(os.path.join(llm_dir, f"token_usage_agent_{self.agent_id}.txt"), 'a') as f:
                                f.write("INPUT PROMPT:\n")
                                for i, msg in enumerate(prompt):
                                    f.write(f"[{msg.get('role', 'unknown').upper()}]\n")
    ...
    def progress2text(self, current_step, satisfied, opponent_grabbed_objects, opponent_last_room,):
        s = f"I've taken {current_step}/{self.max_frames} steps. "
