...
class TransportChallenge(AssetCachedController):
    ...
    def _start_trial_new(self, replicants: Union[int, List[Union[int, np.ndarray, Dict[str, float]]]] = 2, task_type = 'food', random_seed: int = None) -> None:
        ...
        if not os.path.exists(load_path):
            ...
            path_2 = os.path.join(self.data_prefix, f"{self.scene}_{self.layout}_2.json")
            # Fallback to _0 suffix (observed in some datasets like 5a 10_objs)
            path_0 = os.path.join(self.data_prefix, f"{self.scene}_{self.layout}_0.json")
            
            if os.path.exists(path_2):
                load_path = path_2
            elif os.path.exists(path_0):
                load_path = path_0
        ...
        metadata_path_0 = os.path.join(self.data_prefix, f"{self.scene}_{self.layout}_0_metadata.json")
        ...
        elif os.path.exists(metadata_path_0):
            load_count_and_position_path = metadata_path_0
        else:
